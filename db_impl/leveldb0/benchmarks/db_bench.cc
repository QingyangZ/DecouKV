...
// modified by zqy
static bool FLAGS_use_global_index = false;

static bool FLAGS_use_queue_monitor = false;

static bool FLAGS_enable_compaction = false;
...
class KeyBuffer {
 public:
  ...
  Slice slice() const { return Slice(buffer_, FLAGS_key_prefix + 24); }
...
class Benchmark {
 private:
  ...
  void PrintHeader() {
    const int kKeySize = 24 + FLAGS_key_prefix;
    PrintEnvironment();
 ...
 private:
  ...
  void Open() {
    ...
    options.use_global_index = FLAGS_use_global_index;
    options.use_queue_monitor = FLAGS_use_queue_monitor;
    options.enable_compaction = FLAGS_enable_compaction;
...
int main(int argc, char** argv) {
  ...
  FLAGS_use_global_index = leveldb::Options().use_global_index;
  FLAGS_use_queue_monitor = leveldb::Options().use_queue_monitor;
  FLAGS_enable_compaction = leveldb::Options().enable_compaction;
  ...
  for (int i = 1; i < argc; i++) {
    ...
    if (leveldb::Slice(argv[i]).starts_with("--benchmarks=")) {
      ...
      FLAGS_open_files = n;
    } else if (sscanf(argv[i], "--use_global_index=%d%c", &n, &junk) == 1 &&
               (n == 0 || n == 1)) {
      FLAGS_use_global_index = n;
    } else if (sscanf(argv[i], "--use_queue_monitor=%d%c", &n, &junk) == 1 &&
               (n == 0 || n == 1)) {
      FLAGS_use_queue_monitor = n;
    } else if (sscanf(argv[i], "--enable_compaction=%d%c", &n, &junk) == 1 &&
               (n == 0 || n == 1)) {
      FLAGS_enable_compaction = n;
