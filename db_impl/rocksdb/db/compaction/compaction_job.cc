...
Status CompactionJob::Run() {
  ...
  if (compact_->compaction->output_level() == 0 ||
    ...
    std::ofstream fout("/mnt/sde/zqy/data/rocksdb_l0l1l2/l0l1_compaction.txt",
                       std::ios::app);
    fout << start_micros << ',' << env_->NowMicros() << std::endl;
    fout.close();
  } else {
    std::ofstream fout("/mnt/sde/zqy/data/rocksdb_l0l1l2/compaction.txt",
                     std::ios::app);
