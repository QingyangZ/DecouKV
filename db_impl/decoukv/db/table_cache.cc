...
InternalIteratorBase<IndexValue>* TableCache::IndexBlockGet(
    const ReadOptions& options,
    const InternalKeyComparator& internal_comparator,
    const FileMetaData& file_meta, const SliceTransform* prefix_extractor,
    HistogramImpl* file_read_hist, bool skip_filters, int level,
    size_t max_file_size_for_l0_meta_pin, IndexBlockIter* iiter_on_stack) {
  auto& fd = file_meta.fd;
  Status s;
  TableReader* t = fd.table_reader;
  Cache::Handle* handle = nullptr;
  ...
  if (s.ok()) {
    auto iiter = t->GetIndexBlockIter(options, prefix_extractor, iiter_on_stack);
    iiter->SeekToFirst();
    return iiter;
  }
}

Status TableCache::GetIterByIndexBlock(
    const ReadOptions& options,
    const InternalKeyComparator& internal_comparator,
    const FileMetaData& file_meta, const SliceTransform* prefix_extractor,
    HistogramImpl* file_read_hist, bool skip_filters, int level,
    size_t max_file_size_for_l0_meta_pin, IndexValue& value,
    DataBlockIter* iiter) {
  auto& fd = file_meta.fd;
  Status s;
  TableReader* t = fd.table_reader;
  ...
  if (s.ok()) {
    t->GetDataBlockIterByIndex(options, value, iiter);
    // ReleaseHandle(handle);
  }
  return s;
}

Status TableCache::GetByIndexValue(const ReadOptions& options,
                        const InternalKeyComparator& internal_comparator,
                        const FileMetaData& file_meta, const Slice& k,
                        IndexValue& value, GetContext* get_context,
                        const SliceTransform* prefix_extractor,
                        HistogramImpl* file_read_hist, bool skip_filters,
  ...
  if (s.ok()) {
    s = t->GetByIndexValue(options, k, value, get_context, prefix_extractor,
                           skip_filters);
    // ReleaseHandle(handle);
  }
  return s;
