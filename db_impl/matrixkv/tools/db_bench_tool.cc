...
DEFINE_int64(ycsb_workloadb_num, 1000000,"");
DEFINE_int64(ycsb_workloadc_num, 1000000,"");
DEFINE_int64(ycsb_workloadd_num, 1000000,"");
DEFINE_int64(ycsb_workloade_num, 1000000,"");
DEFINE_int64(ycsb_workloadf_num, 1000000,"");

DEFINE_bool(YCSB_uniform_distribution, true, "Uniform key distribution for YCSB");
/////
 ...
 private:
  ...
  Stats RunBenchmark(int n, Slice name,
    ...
    if ( FLAGS_report_ops_latency && (method == &Benchmark::YCSBWorkloadB)) {
      shared.latencys = new uint64_t[FLAGS_ycsb_workloadb_num * n];
      n = n + 1;
      shared.total = n;  //need extra thread to record latency and throughput per second
    }

    if ( FLAGS_report_ops_latency && (method == &Benchmark::YCSBWorkloadC)) {
      shared.latencys = new uint64_t[FLAGS_ycsb_workloadc_num * n];
      n = n + 1;
      shared.total = n;  //need extra thread to record latency and throughput per second
    }

    if ( FLAGS_report_ops_latency && (method == &Benchmark::YCSBWorkloadD)) {
      shared.latencys = new uint64_t[FLAGS_ycsb_workloadd_num * n];
      n = n + 1;
      shared.total = n;  //need extra thread to record latency and throughput per second
    }

    if ( FLAGS_report_ops_latency && (method == &Benchmark::YCSBWorkloadE)) {
      shared.latencys = new uint64_t[FLAGS_ycsb_workloade_num * n];
      n = n + 1;
      shared.total = n;  //need extra thread to record latency and throughput per second
    }

    if ( FLAGS_report_ops_latency && (method == &Benchmark::YCSBWorkloadF)) {
      shared.latencys = new uint64_t[FLAGS_ycsb_workloadf_num * n];
      n = n + 1;
      shared.total = n;  //need extra thread to record latency and throughput per second
    }
...
void YCSBWorkloadB(ThreadState* thread) {
    ...
    int64_t reads_done = 0;
    int64_t writes_done = 0;
    Duration duration(FLAGS_duration, FLAGS_ycsb_workloadb_num);
    ...
    snprintf(msg, sizeof(msg), "( reads:%" PRIu64 " writes:%" PRIu64 \
             " total:%" PRIu64 " found:%" PRIu64 ")",
             reads_done, writes_done, FLAGS_ycsb_workloadb_num, found);
    thread->stats.AddMessage(msg);
  }
  void YCSBWorkloadC(ThreadState* thread) {
    ...
    int64_t reads_done = 0;
    int64_t writes_done = 0;
    Duration duration(FLAGS_duration, FLAGS_ycsb_workloadc_num);
    ...
    snprintf(msg, sizeof(msg), "( reads:%" PRIu64 " writes:%" PRIu64 \
             " total:%" PRIu64 " found:%" PRIu64 ")",
             reads_done, writes_done, FLAGS_ycsb_workloadc_num, found);
    thread->stats.AddMessage(msg);
  }
  void YCSBWorkloadD(ThreadState* thread) {
    ...
    int64_t reads_done = 0;
    int64_t writes_done = 0;
    Duration duration(FLAGS_duration, FLAGS_ycsb_workloadd_num);
    ...
    snprintf(msg, sizeof(msg), "( reads:%" PRIu64 " writes:%" PRIu64 \
             " total:%" PRIu64 " found:%" PRIu64 ")",
             reads_done, writes_done, FLAGS_ycsb_workloadd_num, found);
    thread->stats.AddMessage(msg);
  }
  void YCSBWorkloadE(ThreadState* thread) {
    ...
    Duration duration(FLAGS_duration, FLAGS_ycsb_workloade_num);
    ...
    while (!duration.Done(1)) {
      ...
      long k;
      k = (FLAGS_num - nextValue()) % FLAGS_num;
      // if (FLAGS_YCSB_uniform_distribution){
      //   //Generate number from uniform distribution            
      //   k = thread->rand.Next() % FLAGS_num;
      // } else { //default
    ...
    snprintf(msg, sizeof(msg), "( reads:%" PRIu64 " writes:%" PRIu64 \
             " total:%" PRIu64 " found:%" PRIu64 ")",
             reads_done, scans_done, FLAGS_ycsb_workloade_num, found);
    thread->stats.AddMessage(msg);
  }
/////
  ...
  void YCSBWorkloadF(ThreadState* thread) {
    ...
    int64_t reads_done = 0;
    int64_t writes_done = 0;
    Duration duration(FLAGS_duration, FLAGS_ycsb_workloadf_num);
    ...
    snprintf(msg, sizeof(msg), "( reads:%" PRIu64 " writes:%" PRIu64 \
             " total:%" PRIu64 " found:%" PRIu64 ")",
             reads_done, writes_done, FLAGS_ycsb_workloadf_num, found);
    thread->stats.AddMessage(msg);
  }
...
// optimization, so skip the check of the max level.
  ...
  void CleanCache() {
    auto res1 = system("sync");
    auto res2 = system("echo 3 > /proc/sys/vm/drop_caches");
    sleep(5);
    auto res3 = system("free -h");
