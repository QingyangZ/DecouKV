...
Status DBImpl::PromoteL0(ColumnFamilyHandle* column_family, int target_level) {
  ...
  JobContext job_context(next_job_id_.fetch_add(1), true);
  {
    ...
    for (const auto& f : l0_files) {
      ...
      //   cfd->global_index->edit_files.emplace_back(
      //   GlobalIndex::EditFile(GlobalIndex::EditFile::ISADD, target_level,
      //                         f->fd.GetNumber(), f->smallest, f->largest));
      //   cfd->global_index->edit_files.emplace_back(
      //   GlobalIndex::EditFile(GlobalIndex::EditFile::ISDELETE, 0,
      //                         f->fd.GetNumber(), f->smallest, f->largest));
      // }
    }
