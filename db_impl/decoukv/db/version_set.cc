 ...
 public:
  FilePicker(std::vector<FileMetaData*>* files, const Slice& user_key,
      ...
      : num_levels_(num_levels),
       // changed by zqy
        curr_level_(static_cast<unsigned int>(start_level)),
        returned_file_level_(static_cast<unsigned int>(start_level)),
        hit_file_level_(static_cast<unsigned int>(start_level)),
...
void Version::Get(const ReadOptions& read_options, const LookupKey& k,
  ...
  FilePicker fp(
      ...
      user_comparator(), internal_comparator(), -1);
  FdWithKeyRange* f = fp.GetNextFile();

  while (f != nullptr) {
    // std::cout << "level: " << fp.GetCurrentLevel()
    //           << " number: " << f->fd.GetNumber()
    //           << " range: " << f->smallest_key.ToString() << "~"
    //           << f->largest_key.ToString() << std::endl;
...
Status VersionSet::ProcessManifestWrites(
  ...
  // for (int i = 0; i < static_cast<int>(versions.size()); ++i) {
  //   ColumnFamilyData* cfd = versions[i]->cfd_;
  //   if (cfd->global_index_exist_ && cfd->global_index->global_index_exists_ 
