...
void MemTableList::PickMemtablesToFlush(uint64_t max_memtable_id,
  ...
  for (auto it = memlist.rbegin(); it != memlist.rend(); ++it) {
    ...
    if (!m->flush_in_progress_) {
      ...
      ret->push_back(m);
      if (single_flush) break;  // modified by zqy. Flush one memtable once.
    }
  }
...
Status MemTableList::RemoveMemTable(MemTable* m,
                                  autovector<MemTable*>* to_delete,
...
void MemTableList::RemoveMemTablesOrRestoreFlags(
  ...
  if (s.ok() && !cfd->IsDropped()) {  // commit new state
    while (batch_count-- > 0) {
      ...
      assert(m->file_number_ > 0);
      for (auto mem : m->memtables_) {
        current_->UnrefMemTable(to_delete, mem);
      }
      current_->Remove(m, to_delete);
      // modified by zqy
  ...
  if (s.ok() || s.IsColumnFamilyDropped()) {
    for (size_t i = 0; i != cfds.size(); ++i) {
      ...
      for (auto m : *mems_list[i]) {
        ...
        for (auto mem : m->memtables_) {
          imm->current_->UnrefMemTable(to_delete, mem);
        }
