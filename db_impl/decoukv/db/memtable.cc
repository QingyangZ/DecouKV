...
class MemTableIterator : public InternalIterator {
 public:
  ...
  void* GetNode() override { return iter_->GetNode(); }
  void SeekToHead() override { return iter_->SeekToHead(); }
...
// modified by zqy
bool MemTable::GetByNode(const LookupKey& key, std::string* value,
                           std::string* timestamp, Status* s,
                           MergeContext* merge_context,
                           SequenceNumber* max_covering_tombstone_seq,
                           SequenceNumber* seq, const ReadOptions& read_opts,
                           void** next_node, ReadCallback* callback,
  ...
  if (bloom_filter_ && !may_contain) {
    ...
    *seq = kMaxSequenceNumber;
    ...
    GetFromTableByNode(key, *max_covering_tombstone_seq, do_merge, callback,
                 is_blob_index, value, timestamp, s, merge_context, seq,
                 &found_final_value, &merge_in_progress, next_node);
  }
...
void MemTable::Compact(SequenceNumber seq, MemTable* last_table) {
  table_->Compact(seq, last_table->table_.get()); 
}
...
void MemTable::GetFromTableByNode(const LookupKey& key,
                                  SequenceNumber max_covering_tombstone_seq,
                                  ...
                                  bool* merge_in_progress, void** next_node) {
  Saver saver;
  saver.status = s;
  ...
  table_->GetByNode(key, &saver, SaveValue, next_node);
  *seq = saver.seq;
}
