...
bool DBImpl::IsNeedIndexMerge(ColumnFamilyData* cfd) {
  const auto& memlist = cfd->imm()->current_->memlist_;
  // if (memlist.size() == 1) cfd_->imm()->current_->Unref();
  // std::cout << "memlist size: " << memlist.size() << std::endl;
  return (CalMergeQueueLength(cfd) >=
          cfd->GetLatestMutableCFOptions()->merge_trigger_num);
}

bool DBImpl::IsScheduleFlush(ColumnFamilyData* cfd) {
  const auto& memlist = cfd->imm()->current_->memlist_;
  for (auto it = memlist.rbegin(); it != memlist.rend(); ++it) {
    ...
    break; // only one file flushing
  }
  return false;
}

bool DBImpl::IsNeedFlush(ColumnFamilyData* cfd) {
  const auto& memlist = cfd->imm()->current_->memlist_;
  for (auto it = memlist.rbegin(); it != memlist.rend(); ++it) {
    MemTable* m = *it;
    
    if (m->count_table_ >=
                cfd->GetLatestMutableCFOptions()->slow_flush_trigger_num &&
            !m->is_schflushed_) {
      // m->mutex_.lock();
      m->is_schflushed_ = true;
      // std::cout << "need flush" << std::endl;
      return true;
    }
    break; // only one file flushing
  }
...
int DBImpl::CalMergeQueueLength(ColumnFamilyData* cfd) {
  const auto& memlist = cfd->imm()->current_->memlist_;
  int count = 0;
  for (auto it = memlist.rbegin(); it != memlist.rend(); ++it) {
    MemTable* m = *it;
...
int DBImpl::CalFlushQueueLength(ColumnFamilyData* cfd) {
  const auto& memlist = cfd->imm()->current_->memlist_;
  int count = 0;
  for (auto it = memlist.rbegin(); it != memlist.rend(); ++it) {
    MemTable* m = *it;
...
void DBImpl::ControllerWork(ColumnFamilyData* cfd) {
  auto now_time = std::chrono::high_resolution_clock::now();
  std::chrono::duration<double> ntime = now_time - start_time_;
  auto diff_ntime = std::chrono::duration_cast<std::chrono::seconds>(ntime);
  if (CalMergeQueueLength(cfd) >
          1.5 * cfd->GetLatestMutableCFOptions()->merge_trigger_num &&
      CalFlushQueueLength(cfd) > 2) {
    // Both CPU bound and I/O bound
    cfd->mutable_cf_options_.slow_flush_trigger_num =
        std::min(32, cfd->mutable_cf_options_.slow_flush_trigger_num * 2);
    cfd->mutable_cf_options_.merge_trigger_num =
        std::min(16, cfd->mutable_cf_options_.merge_trigger_num + 2);
    is_timer_start_.store(false);
  } else if (CalFlushQueueLength(cfd) > 2) {
    // I/O bound
    cfd->mutable_cf_options_.slow_flush_trigger_num =
        std::min(32, cfd->mutable_cf_options_.slow_flush_trigger_num * 2);
    cfd->mutable_cf_options_.merge_trigger_num =
        std::max(2, cfd->mutable_cf_options_.merge_trigger_num - 2);
    is_timer_start_.store(false);
  } else if (CalMergeQueueLength(cfd) >
             1.5 * cfd->GetLatestMutableCFOptions()->merge_trigger_num) {
    // CPU bound
    cfd->mutable_cf_options_.slow_flush_trigger_num =
        std::max(4, cfd->mutable_cf_options_.slow_flush_trigger_num / 2);
    cfd->mutable_cf_options_.merge_trigger_num =
        std::min(16, cfd->mutable_cf_options_.merge_trigger_num + 2);
    is_timer_start_.store(false);
  } else {
    ...
    if (!is_timer_start_.load()) {
      timer_ = now_time;
      is_timer_start_.store(true);
    } else {
      std::chrono::duration<double> diff = now_time - timer_;
      auto diff_seconds = std::chrono::duration_cast<std::chrono::seconds>(diff);
      if (diff_seconds.count() > 100) {
        cfd->mutable_cf_options_.slow_flush_trigger_num =
            std::max(8, cfd->mutable_cf_options_.slow_flush_trigger_num / 2);
        cfd->mutable_cf_options_.merge_trigger_num =
            std::max(2, cfd->mutable_cf_options_.merge_trigger_num - 2);
        timer_ = now_time;
        is_timer_start_.store(false);
      }
      // cfd->ioptions_()->clock->NowMicros/1000000;
    }
  }
  std::ofstream fout("/mnt/sde/zqy/data/index/tuning.txt", std::ios::app);

  fout << diff_ntime.count() << '\t' << cfd->mutable_cf_options_.merge_trigger_num
       << '\t' << cfd->mutable_cf_options_.slow_flush_trigger_num << std::endl;
  fout.close();

  std::ofstream fout2("/mnt/sde/zqy/data/index/queue_length.txt", std::ios::app);
  fout2 << diff_ntime.count() << '\t' << CalMergeQueueLength(cfd)
       << '\t' << CalFlushQueueLength(cfd) << std::endl;
  fout2.close();
}
...
Status DBImpl::FlushMemTableToOutputFile(
  ...
  // if (!cfd->queue_moniter.l0_queue.cal_start_) {
  //   cfd->queue_moniter.l0_queue.cal_start_ = true;
  //   cfd->queue_moniter.l0_queue.arrive_ = std::chrono::system_clock::now();
  // }
  // std::cout << cfd->imm()->NumNotFlushed() << std::endl;
  FlushJob flush_job(
  ...
  if (s.ok()) {
    // if (bg_flush_scheduled_ + unscheduled_flushes_ > 2) {
    //   cfd->mutable_cf_options_.slow_flush_trigger_num =
    //       std::max(1, cfd->mutable_cf_options_.slow_flush_trigger_num / 2);
    //   cfd->mutable_cf_options_.merge_trigger_num =
    //       std::min(8, cfd->mutable_cf_options_.merge_trigger_num + 2);
    // } else if (bg_flush_scheduled_ == 0) {
    //   cfd->mutable_cf_options_.slow_flush_trigger_num =
    //       std::min(64, cfd->mutable_cf_options_.slow_flush_trigger_num * 2);
    //   cfd->mutable_cf_options_.merge_trigger_num =
    //       std::max(2, cfd->mutable_cf_options_.merge_trigger_num - 2);
    // }
    ControllerWork(cfd);
    if (IsScheduleFlush(cfd) ||
        cfd->GetLatestMutableCFOptions()->slow_flush_trigger_num == 0) {
      flush_job.PickMemTable();
      // std::cout << "shcedule flush" << std::endl;
      need_cancel = true;
    } else {
      if (IsNeedIndexMerge(cfd)) {
        // IndexMerge
        flush_job.IndexMerge();
        if (IsNeedFlush(cfd)) {
          unscheduled_flushes_++;
          MaybeScheduleFlushOrCompaction();
        }
        if (IsNeedIndexMerge(cfd)) {
          std::cout << "Double IndexMerge" << std::endl;
          flush_job.IndexMerge();
        }
      ...
      bg_cv_.SignalAll();
      cfd->imm()->num_flush_not_started_--;
      return IOStatus::OK();
    }
  }
  ...
  // for (auto it = memlist.rbegin(); it != memlist.rend(); ++it) {
  //   MemTable* m = *it;
  //   if (cfd->GetLatestMutableCFOptions()->slow_flush_trigger_num != 0) {
  ...
  //   }
    ...
    //   if (m->is_flushed_) {
    //     for (auto blob_number : m->blob_files_) {
    //       auto it2 = blob_files_.find(blob_number);
    //       if (it2 == blob_files_.end()) {
    //         return Status::NotFound("Blob Not Found as blob file missing");
    //       }
    //       auto blob_file = it2->second;
  ...
  // for (auto deleted_number : flush_job.deleted_blob_numbers) {
  //   blob_files_.erase(deleted_number);
  // }
  if (s.ok()) {
  ...
  //   cfd->global_index->add_files_.emplace(
  //       file_meta.fd.GetNumber(),
  //       GlobalIndex::EditFile(0, file_meta.smallest, file_meta.largest));
  // }
  
  // if (cfd->queue_moniter.l0_arrive_ != 0) {
  //   // cfd->queue_moniter.PrintTime(cfd->queue_moniter.l0_service_, current_);
  //   cfd->queue_moniter.l0_arrive_rate_ =
  //       cfd->queue_moniter.CalRate(cfd->queue_moniter.l0_arrive_, current_, 1);
  //   std::cout << "L0 arrive rate: " << cfd->queue_moniter.l0_arrive_rate_ << " "
  //             << cfd->current()->storage_info()->NumLevelFiles(0) << std::endl;
  // }
  // cfd->queue_moniter.l0_arrive_ = current_;
  // if (!cfd->queue_moniter.l0_write_stall &&
  //         cfd->current()->storage_info()->NumLevelFiles(0) >=
  //             cfd->GetCurrentMutableCFOptions()
  //                 ->level0_slowdown_writes_trigger) {
  //   cfd->queue_moniter.l0_write_stall = true;
  // }
  // auto current_ = std::chrono::system_clock::now();
  // cfd->queue_moniter.mem_queue.service_rate_ = cfd->queue_moniter.CalRate(
  //     cfd->queue_moniter.mem_queue.service_, current_, 1);
  // cfd->queue_moniter.mem_queue.service_ = current_;
  // cfd->queue_moniter.mem_queue.UpdateState();
  // std::ofstream fout("MemData.txt", std::ios::app);
  // fout << cfd->queue_moniter.DeltaTime(cfd->queue_moniter.start_time, current_)
  //      << "," << cfd->current()->storage_info()->NumLevelFiles(0) << ","
  //      << cfd->queue_moniter.mem_queue.arrive_rate_ << ","
  //      << cfd->queue_moniter.mem_queue.service_rate_ << ","
  //      << cfd->GetLatestMutableCFOptions()->write_buffer_size << ','
  //      << immutable_db_options_.env->GetBackgroundThreads(Env::Priority::HIGH)
  //      << ','
  //      << immutable_db_options_.env->GetBackgroundThreads(Env::Priority::LOW)
  //      << std::endl;
  // if (cfd->GetCurrentMutableCFOptions()->use_queue_monitor) {
  //   switch (cfd->queue_moniter.mem_queue.state) {
  ...
  // if (!cfd->queue_moniter.l0_queue.cal_done_) {
    
  //   if (cfd->queue_moniter.l0_service_num_ < cfd->queue_moniter.L0_SERVICE_NUM) {
  //     cfd->queue_moniter.l0_service_num_++;
  //   } else {
  //     cfd->queue_moniter.l0_queue.arrive_rate_ =
  //         cfd->queue_moniter.CalRate(cfd->queue_moniter.l0_queue.arrive_, current_,
  //                                    cfd->queue_moniter.L0_SERVICE_NUM);
  //     // std::cout << "L0 arrive rate: " << cfd->queue_moniter.l0_queue.arrive_rate_
  //     //           << " " << cfd->current()->storage_info()->NumLevelFiles(0)
  //     //           << std::endl;
  //     cfd->queue_moniter.l0_service_num_ = 0;
  //     cfd->queue_moniter.l0_queue.cal_done_ = true;
  //   }
  // }

  // std::ofstream fout1("MemData.txt", std::ios::app);
  // fout1 << cfd->queue_moniter.DeltaTime(cfd->queue_moniter.start_time,
  //                                       current_)
  //       << "\t" << cfd->imm()->NumNotFlushed() << "\t" << std::endl;
  // fout1.close();
  ...
  //             1024 / 1024 / 1024
  //      << std::endl;
  // // cfd->queue_moniter.PrintTime(cfd->queue_moniter.start_time,
  // // current_);
  // fout.close();
  // if (cfd->queue_moniter.mem_service_ != 0) {
  //   // std::cout << "service: ";
  //   // cfd->queue_moniter.PrintTime(cfd->queue_moniter.mem_service_,
  //   // current_);
  // }
  // if (cfd->imm()->NumNotFlushed() == 0) {
  //   cfd->queue_moniter.mem_service_ = 0;
  // } else {
  //   cfd->queue_moniter.mem_service_ = current_;
  // }
  // std::cout << "num_not_flushed: " << cfd->imm()->NumNotFlushed() << std::endl;
  return s;
}
...
Status DBImpl::ReFitLevel(ColumnFamilyData* cfd, int level, int target_level) {
  ...
  if (to_level != level) {
    ...
    for (const auto& f : vstorage->LevelFiles(level)) {
      ...
      if (cfd->global_index->global_index_exists_) {
        cfd->global_index->delete_files_.emplace(
            f->fd.GetNumber(),
            GlobalIndex::EditFile(level, f->smallest, f->largest));
        cfd->global_index->add_files_.emplace(
            f->fd.GetNumber(),
            GlobalIndex::EditFile(to_level, f->smallest, f->largest));
      }
    }
    ROCKS_LOG_DEBUG(immutable_db_options_.info_log,
...
Status DBImpl::FlushMemTable(ColumnFamilyData* cfd,
  ...
  autovector<uint64_t> memtable_ids_to_wait;
  {
    ...
    if (s.ok() && !flush_reqs.empty()) {
      ...
      cfd->mutable_cf_options_.slow_flush_trigger_num = 0;
      MaybeScheduleFlushOrCompaction();
    }
...
DBImpl::BGJobLimits DBImpl::GetBGJobLimits(int max_background_flushes,
  ...
  if (max_background_flushes == -1 && max_background_compactions == -1) {
    ...
    res.max_flushes = std::max(4, max_background_jobs / 4);
    res.max_compactions = std::max(1, max_background_jobs - res.max_flushes);
  } else {
    // compatibility code in case users haven't migrated to max_background_jobs,
    // which automatically computes flush/compaction limits
    res.max_flushes = std::max(3, max_background_flushes);
    res.max_compactions =
...
Status DBImpl::BackgroundFlush(bool* made_progress, JobContext* job_context,
  ...
  *reason = FlushReason::kOthers;
  ...
  if (!bg_flush_args.empty()) {
    ...
    // if (bg_flush_args[0].cfd_->global_index->global_index_exists_) {
    //   if (bg_flush_args[0].cfd_->global_index->added_filenumber.size() > 0) {
    //     ReadOptions read_options;
    //     bg_flush_args[0].cfd_->global_index->AddFiles(read_options);
    //   }
    // }
...
Status DBImpl::BackgroundCompaction(bool* made_progress,
  ...
  *made_progress = false;
  ...
  // if (c->output_level() == 1) {
  //   auto current_ = std::chrono::system_clock::now();
  //   c->column_family_data()->queue_moniter.l0_queue.service_ = current_;
  //   std::cout << "L0 compaction!" << std::endl;
  // }
  IOStatus io_s;
  ReadOptions read_options;
  if (!c) {
    ...
    for (const auto& f : *c->inputs(0)) {
      ...
      if (c->column_family_data()->global_index->global_index_exists_) {
        c->column_family_data()->global_index->delete_files_.emplace(
            f->fd.GetNumber(),
            GlobalIndex::EditFile(c->level(), f->smallest, f->largest));
      }
    }
    ...
    for (unsigned int l = 0; l < c->num_input_levels(); l++) {
      ...
      for (size_t i = 0; i < c->num_input_files(l); i++) {
        ...
        if (c->column_family_data()->global_index->global_index_exists_) {
          c->column_family_data()->global_index->delete_files_.emplace(
              f->fd.GetNumber(),
              GlobalIndex::EditFile(c->level(l), f->smallest, f->largest));

          c->column_family_data()->global_index->add_files_.emplace(
              f->fd.GetNumber(),
              GlobalIndex::EditFile(c->output_level(), f->smallest,
                                    f->largest));
        }
