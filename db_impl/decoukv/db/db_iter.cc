...
bool DBIter::SetBlobValueIfNeeded(const Slice& user_key,
  ...
  Status s = blob_index_.DecodeFrom(blob_index);
  if (!s.ok()) {
    return false;
  }
  CompressionType compression_type = kNoCompression;
  s = db_impl_->GetRawBlobFromFile(user_key,blob_index_.file_number(),blob_index_.offset(),blob_index_.size(),&blob_value_,&compression_type);
