...
class DBImpl : public DB {
 ...
 private:
  ...
  bool IsNeedIndexMerge(ColumnFamilyData* cfd);
  bool IsNeedFlush(ColumnFamilyData* cfd);
  bool IsScheduleFlush(ColumnFamilyData* cfd);
  int CalMergeQueueLength(ColumnFamilyData* cfd);
  int CalFlushQueueLength(ColumnFamilyData* cfd);
  void ControllerWork(ColumnFamilyData* cfd);
  std::chrono::high_resolution_clock::time_point timer_;
  std::chrono::high_resolution_clock::time_point start_time_;
  std::atomic<bool> is_timer_start_;
  ...
  void RegisterBlobFile(std::shared_ptr<blob_db::BlobFile> blob_file);
  ...
  int blob_fd_ = -1;
  // add a new Blob File
  std::shared_ptr<blob_db::BlobFile> NewBlobFile(bool has_ttl,
                                        const ExpirationRange& expiration_range,
                                        const std::string& reason);

  // ..returns a Writer object for the file. If writer is not
  // already present, creates one. Needs Write Mutex to be held
  Status CheckOrCreateWriterLocked(const std::shared_ptr<blob_db::BlobFile>& blob_file,
                                   std::shared_ptr<BlobLogWriter>* writer);

  // hold write mutex on file and call
  // creates a sequential (append) writer for this blobfile
  Status CreateWriterLocked(const std::shared_ptr<blob_db::BlobFile>& bfile);

  Status CloseBlobFile(std::shared_ptr<blob_db::BlobFile> bfile);
  // select a blob file to write
  Status SelectBlobFile(std::shared_ptr<blob_db::BlobFile>* bfile, uint64_t size);

  Status PutBlobValue(const Slice& key, const Slice& value, std::string* index_entry,uint32_t column_family);

  Status GetBlobFileReader(const std::shared_ptr<blob_db::BlobFile>& bfile, std::shared_ptr<RandomAccessFileReader>* reader);
  ...
  // tht update: counter for blob file number
  std::atomic<uint64_t> next_blob_number_;

  // tht update: entire metadata of all the BLOB files memory
  std::map<uint64_t, std::shared_ptr<blob_db::BlobFile>> blob_files_;

  // tht update: Total size of all live blob files (i.e. exclude obsolete files).
  std::atomic<uint64_t> total_blob_size_;

  // tht update: the opened blob file
  std::shared_ptr<blob_db::BlobFile> open_blob_file_;
