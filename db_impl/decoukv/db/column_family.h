...
class ColumnFamilyData {
 public:
  ...
  void SetWriteStallParameter(int compaction_trigger, int slowdown_trigger,
                              int stop_trigger, size_t write_buffer_size) {
    if (compaction_trigger >= 2 && compaction_trigger <= 20) {
      mutable_cf_options_.level0_file_num_compaction_trigger =
          compaction_trigger;
    }
    if (slowdown_trigger >= 20 && slowdown_trigger <= 100) {
      mutable_cf_options_.level0_slowdown_writes_trigger = slowdown_trigger;
    }
    if (stop_trigger >= 36 && stop_trigger <= 180) {
      mutable_cf_options_.level0_stop_writes_trigger = stop_trigger;
    }
    if (write_buffer_size >= 64 << 20 && write_buffer_size <= 512 << 20) {
      mutable_cf_options_.write_buffer_size = write_buffer_size;
    }
  }
  ...
  bool global_index_exist_ = false;
  QueueMonitor queue_moniter;
  KeyTable* key_table;
