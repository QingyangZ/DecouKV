  ...
  *reason = FlushReason::kOthers;
  ...
  if (!bg_flush_args.empty()) {
    ...
    if (immutable_db_options_.nvm_setup != nullptr &&
      ...
      std::ofstream fout("/mnt/sde/zqy/data/matrixkv/flush.txt",
                         std::ios::app);
      auto start_time = env_->NowMicros();
      status = FlushMemTablesToNvm(bg_flush_args, made_progress, job_context,
                                   log_buffer);
      fout << start_time << ',' << env_->NowMicros() << std::endl;
      fout.close();
    } else {
