...
struct ColumnFamilyOptions : public AdvancedColumnFamilyOptions {
  ...
  //
  // Dynamically changeable through SetOptions() API
  int level0_file_num_compaction_trigger = 1;

  // modified by zqy
  // If true, the database will use global index for speeding up the query
  bool use_global_index = false;
  // If true, the database will use use queue monitor for speeding up write
  // stall.
  bool use_queue_monitor = true;

  int slow_flush_trigger_num = 8;

  int merge_trigger_num = 2;
  ...
  //
  // Dynamically changeable through SetOptions() API
  uint64_t max_bytes_for_level_base = 10 * 1073741824ull;
...
struct DBOptions {
  ...
  // blob file path
  std::string blob_path = "/mnt/pmem0/test";
};
...
struct WriteOptions {
  ...
  uint64_t nvm_stall_trigger = 20 * 1073741824ull; 
