...
Status DBImpl::NewDB(std::vector<std::string>* new_filenames) {
  ...
  const std::string manifest = DescriptorFileName(dbname_, 1);
  {
    ...
    std::ofstream fout1("/mnt/sde/zqy/data/rocksdb_l0l1l2/compaction.txt");
    fout1 << env_->NowMicros() << std::endl;
    fout1.close();
    std::ofstream fout2("/mnt/sde/zqy/data/rocksdb_l0l1l2/flush.txt");
    fout2 << env_->NowMicros() << std::endl;
    fout2.close();
    std::ofstream fout3("/mnt/sde/zqy/data/rocksdb_l0l1l2/l0l1_compaction.txt");
    fout3 << env_->NowMicros() << std::endl;
    fout3.close();
