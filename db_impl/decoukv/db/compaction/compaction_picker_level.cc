...
Compaction* LevelCompactionBuilder::PickCompaction() {
  ...
  // if (start_level_ == 0 && mutable_cf_options_.use_queue_monitor) {
  //   start_level_inputs_.clear();
  //   start_level_inputs_.level = 0;
  //   const std::vector<FileMetaData*>& level_files =
  //       vstorage_->LevelFiles(start_level_);
  //   int count = 0;
  //   for (unsigned int i = level_files.size() - 1; i >= 0; i--) {
  //     auto* f = level_files[i];
  //     start_level_inputs_.files.push_back(f);
  //     count++;
  //     if (count >= mutable_cf_options_.level0_compaction_file_num) {
  //       break;
  //     }
  //   }
  // } else {
...
uint32_t LevelCompactionBuilder::GetPathId(
  ...
  if (/*ioptions.cf_paths.size() > 1 && */level == 0) return 1;
  else return 0;
