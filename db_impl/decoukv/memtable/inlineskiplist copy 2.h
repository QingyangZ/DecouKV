...
class InlineSkipList {
 ...
 public:
  ...
  class Iterator {
   public:
    ...
    // modified by zqy
    void ReplaceValue(/*Slice& value,*/ void* next_node);
    void SeekToHead();
    bool IsHead();
    void* GetNode();
    void SeekByNode(const char* target, void** next_node);
  ...
  //                   const Options& options_, MergeableBloom* bloom_);
  void PreNext(Node** pre, int height);
  bool Compact(void* vlist, SequenceNumber snum);
  bool Compact(void* vlist, bool frontlink);
  bool SubMerge(InlineSkipList<Comparator>* list, SequenceNumber snum, Node* start_node, Node* end_node);

  void Insert(InlineSkipList<Comparator>::Node* n, Node** prev);
  ...
  void Decode(const char* key, SequenceNumber* snum) const {
    uint32_t key_length = 0;
    const char* key_ptr = GetVarint32Ptr(key, key + 5, &key_length);
    assert(key_length >= 8);
    const uint64_t tag = DecodeFixed64(key_ptr + key_length - 8);
    ValueType type;
    UnPackSequenceAndType(tag, snum, &type);
  }

  int NewCompare(const char* akey, const char* bkey, bool hasseq,
                 const SequenceNumber snum) const {
    int r = compare_(akey, bkey);
    if (r < 0) {
      r = 0b01;
    } else if (r == 0) {
      r = 0b10;
  ...
  bool NewCompare(const char* akey, const char* bkey) const {
    int r = compare_(akey, bkey);
    if (r == 0) {
      // TODO:
      SequenceNumber anum;
 ...
 private:
  ...
  // modified by zqy 
  Node* FindGreaterOrEqualByNode(const char* key, void** next_node) const;
...
struct InlineSkipList<Comparator>::Node {
  ...
  void StashHeight(const int height) {
    ...
    memcpy(static_cast<void*>(&next_[2]), &height, sizeof(int));
  }
  ...
  const char* Key() const { return reinterpret_cast<const char*>(&next_[3]); }

  // modified by zqy
  void ReplaceValue(/*Slice& value, */Node* next_node) {
    (&next_[1])->store(next_node, std::memory_order_release);
  }
 ...
 private:
  ...
  std::atomic<Node*> next_[3];
  // std::atomic<Node*> next_ptr;
};
...
// modified by zqy
template <class Comparator>
inline void InlineSkipList<Comparator>::Iterator::ReplaceValue(/*Slice& value,*/
                                                               void* next_node) {
  node_->ReplaceValue(/*value, */(Node*)next_node);
  Node* test_node = reinterpret_cast<Node*>(next_node);
  if (test_node != nullptr) test_node->Next(0);
}

template <class Comparator>
inline void InlineSkipList<Comparator>::Iterator::SeekToHead() {
  node_ = list_->head_;
}

template <class Comparator>
inline bool InlineSkipList<Comparator>::Iterator::IsHead() {
  return node_ == list_->head_;
}

template <class Comparator>
inline void* InlineSkipList<Comparator>::Iterator::GetNode() {
  return node_;
}
...
// modified by zqy
template <class Comparator>
inline void InlineSkipList<Comparator>::Iterator::SeekByNode(const char* target,
                                                             void** start_node) {
  node_ = list_->FindGreaterOrEqualByNode(target, start_node);
}
...
InlineSkipList<Comparator>::FindGreaterOrEqualByNode(const char* key,
                                                     void** start_node) const {
  // Note: It looks like we could reduce duplication by implementing
  // this function as FindLessThan(key)->Next(0), but we wouldn't be able
  // to exit early on equality and the result wouldn't even be correct.
  // A concurrent insert might occur after FindLessThan(key) but before
  // we get a chance to call Next(0).
  Node* x = reinterpret_cast<Node*>(*start_node);
  int level;
  if (x == nullptr) {
    x = head_;
  ...
  while (true) {
    ...
    if (cmp == 0 || (cmp > 0 && level == 0)) {
      *start_node = (cmp == 0) ? next->GetNextNode() : x->GetNextNode();
      Node* test_node = reinterpret_cast<Node*>(*start_node);
...
template <class Comparator>
void InlineSkipList<Comparator>::PreNext(Node** pre, int height) {
  Node* n = pre[0]->Next(0);
...
template <class Comparator>
int InlineSkipList<Comparator>::NewCompare(const Node* a, const Node* b, bool hasseq, SequenceNumber snum) const {
  if (a == nullptr || b == nullptr) {
    return 0;
  } else {
    return NewCompare(a->Key(), b->Key(), hasseq, snum);
  }
}

template <class Comparator>
bool InlineSkipList<Comparator>::NewCompare(const Node* a, const Node* b) const {
  if (a == nullptr || b == nullptr) {
    return false;
  } else {
    return NewCompare(a->Key(), b->Key());
  }
}

template <class Comparator>
void InlineSkipList<Comparator>::Insert(InlineSkipList<Comparator>::Node* n, Node** prev) {
  Node* x = FindLessThan(n->Key(), prev);

  assert(x == nullptr || x == head_ || !Equal(n->Key(), x->Key()));

  int height = n->UnstashHeight();
...
template <class Comparator>
void InlineSkipList<Comparator>::DeleteNode(Node** pre, Node* n) {
  for (int i = 0; i < n->UnstashHeight(); i++) {
    pre[i]->SetNext(i, n->Next(i));
  }
}

template <class Comparator>
bool InlineSkipList<Comparator>::Compact(void* vlist, SequenceNumber snum) {
  InlineSkipList<Comparator>* list = (InlineSkipList<Comparator>*)vlist;
  Node* split_node = list->head_;
  std::vector<std::thread> threads;
  int num = 0;
  int split_height = list->max_height_;
  while (num / 4 < 1) {
    split_height--;
    while (split_node != nullptr) {
      num++;
      split_node = split_node->Next(split_height);
    }
    num = 0;
    list->head_;
  }
 
  split_node = list->head_;
  Node* start_node = split_node->Next(0);
  for (int j = 0; j < 3; j++) {
    for (int i = 0; i < num / 4; i++) {
      split_node = split_node->Next(split_height);
    }
    SubMerge(list, snum, start_node, split_node);
    threads.push_back(std::thread(SubMerge, list, snum, start_node, split_node));
    start_node = split_node;
  }
  SubMerge(list, snum, start_node, nullptr);
  
  std::cout << "0 " << num << std::endl;
}

template <class Comparator>
bool InlineSkipList<Comparator>::SubMerge(InlineSkipList<Comparator>* list, SequenceNumber snum, Node* start_node, Node* end_node) {
  Node* x = start_node;
  Node *y, *ypre[kMaxHeight_], *xpre[kMaxHeight_];
  for (int i = 0; i < kMaxHeight_; i++) {
  ...
  // Set Largest
  for (int i = 0; i < GetMaxHeight(); i++) {
    if (largest[i] != head_ && NewCompare(ypre[i], largest[i], false, 0) == 0b11) {
      largest[i] = ypre[i];
    }
  }
...
template <class Comparator>
bool InlineSkipList<Comparator>::Compact(void* vlist, bool frontlink) {
  InlineSkipList<Comparator>* list = (InlineSkipList<Comparator>*)vlist;
  Node *pre[kMaxHeight_];
