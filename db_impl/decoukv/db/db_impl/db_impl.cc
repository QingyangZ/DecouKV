...
Status DBImpl::GetImpl(const ReadOptions& read_options, const Slice& key,
  ...
  if (!skip_memtable) {
    ...
    if (get_impl_options.get_value) {
      ...
      if (done) {
        Slice blob_index_entry(get_impl_options.value->data(),
                               get_impl_options.value->size());
        ...
        s = GetRawBlobFromFile(key, blob_index.file_number(),
                               blob_index.offset(), blob_index.size(),
                               get_impl_options.value, &compression_type);
      }
    } else {
  ...
  // modified by zqy
  // if (!done && sv->cfd->initial_cf_options().use_queue_monitor) {
  //   if (sv->cfd->key_table->Get(lkey, get_impl_options.value->GetSelf(), timestamp,
  ...
  if (!done) {
    ...
    if (sv->cfd->global_index->global_index_exists_) {
      
      sv->cfd->global_index->GetFromGlobalIndex(
      ...
      sv->current->Get(
          ...
          get_impl_options.get_value ? get_impl_options.is_blob_index : nullptr,
          get_impl_options.get_value);
      // std::cout << "R:" << *get_impl_options.value->GetSelf() << std::endl;
    }
    RecordTick(stats_, MEMTABLE_MISS);
  }
...
Status DBImpl::CreateWriterLocked(
    const std::shared_ptr<blob_db::BlobFile>& bfile) {
  std::string fpath(bfile->PathName());
  std::unique_ptr<FSWritableFile> wfile;
  EnvOptions env_options(initial_db_options_);
  env_options.bytes_per_sync = 512 * 1024;
  ...
  auto statistics = initial_db_options_.statistics.get();
  constexpr bool do_flush = true;
  auto clock = env_->GetSystemClock().get();
  bfile->log_writer_ = std::make_shared<BlobLogWriter>(
      std::move(fwriter), clock, statistics, bfile->file_number_,
      initial_db_options_.use_fsync, do_flush, boffset);
  bfile->log_writer_->last_elem_type_ = et;

  return s;
}

Status DBImpl::CheckOrCreateWriterLocked(
    const std::shared_ptr<blob_db::BlobFile>& blob_file,
...
std::shared_ptr<blob_db::BlobFile> DBImpl::NewBlobFile(
    bool has_ttl, const ExpirationRange& expiration_range,
    const std::string& reason) {
  assert(has_ttl == (expiration_range.first || expiration_range.second));

  uint64_t file_num = next_blob_number_++;

  const uint32_t column_family_id =
      static_cast<ColumnFamilyHandleImpl*>(DefaultColumnFamily())->GetID();
  auto blob_file = std::make_shared<blob_db::BlobFile>(
      nullptr, initial_db_options_.blob_path, file_num,
      initial_db_options_.info_log.get(), column_family_id, kNoCompression,
      has_ttl, expiration_range);
  /*ROCKS_LOG_DEBUG(db_options_.info_log, "New blob file created: %s
  reason='%s'", blob_file->PathName().c_str(), reason.c_str());
  LogFlush(db_options_.info_log);*/
  return blob_file;
}

Status DBImpl::CreateBlobFileAndWriter(
    bool has_ttl, const ExpirationRange& expiration_range,
    const std::string& reason, std::shared_ptr<blob_db::BlobFile>* blob_file,
    std::shared_ptr<BlobLogWriter>* writer) {
  //assert(has_ttl == (expiration_range.first || expiration_range.second));
  assert(blob_file);
  assert(writer);
  ...
  *blob_file = NewBlobFile(has_ttl, expiration_range, reason);
  ...
  if (blob_fd_ != -1) {
    if (close(blob_fd_) == -1) {
      fprintf(stderr, "close fails, error code: %d, error msg: %s\n", errno,
            strerror(errno));
    }
  }
  blob_fd_ = open((*blob_file)->PathName().data(), O_RDWR, 0777);
  if (blob_fd_ == -1) {
    // 打开文件失败，处理错误
    fprintf(stderr, "open fails, error code: %d, error msg: %s\n", errno,
            strerror(errno));
  }
  return s;
}

void DBImpl::RegisterBlobFile(std::shared_ptr<blob_db::BlobFile> blob_file) {
  ...
  blob_files_.insert(
      it, std::map<uint64_t, std::shared_ptr<blob_db::BlobFile>>::value_type(
              blob_file_number, std::move(blob_file)));
}

Status DBImpl::CloseBlobFile(std::shared_ptr<blob_db::BlobFile> bfile) {
  assert(bfile);
...
Status DBImpl::SelectBlobFile(std::shared_ptr<blob_db::BlobFile>* bfile, uint64_t size) {
  Status s;
  if(!open_blob_file_) {
    std::shared_ptr<BlobLogWriter> bwriter;
    s = CreateBlobFileAndWriter(false,ExpirationRange(),"create new blob file",
      bfile,&bwriter);
    if (!s.ok()) {
      return s;
    }
    RegisterBlobFile(*bfile);
    open_blob_file_ = *bfile;
    for (ColumnFamilyData* cfd : *versions_->GetColumnFamilySet()) {
      cfd->mem()->AddBlobFileNumber(open_blob_file_->BlobFileNumber());
    }
  }
  else {
    *bfile = open_blob_file_;
  }
  return s;
}

Status DBImpl::PutBlobValue(const Slice& key, const Slice& value,
                            std::string* index_entry, uint32_t column_family) {
  size_t size = key.size() + value.size();
  Status status;
  std::shared_ptr<blob_db::BlobFile> bfile;
  std::shared_ptr<BlobLogWriter> bwriter;
  uint64_t blob_offset = 0;
  uint64_t key_offset = 0;
  std::string headerbuf;

  // 首先要选择blob file并创建writer
  status = SelectBlobFile(&bfile, (uint64_t)size);
  status = CheckOrCreateWriterLocked(bfile, &bwriter);

  // 向blob file中写入header，在写入之前可能要加锁？
  WriteLock file_lock(&bfile->mutex_);
  BlobLogWriter::ConstructBlobHeader(&headerbuf, key, value,
                                     blob_db::kNoExpiration);
  
  uint64_t size_put = headerbuf.size() + key.size() + value.size();
  std::string buf;
  buf.append(headerbuf.data(), headerbuf.size());
  buf.append(key.data(), key.size());
  buf.append(value.data(), value.size());
  assert(blob_fd_ != -1);
  ssize_t n = pwrite(blob_fd_, buf.data(), size_put, bwriter->block_offset_);
  if (n == -1) {
    // handle error
  ...
  if (false){
    int res_sync = fdatasync(blob_fd_);
    if (res_sync == -1) {
  ...
  key_offset = bwriter->block_offset_ + BlobLogRecord::kHeaderSize;
  blob_offset = key_offset + key.size();
  bwriter->block_offset_ = blob_offset + value.size();

  // 向blob file中写入key和value
  // status = bwriter->EmitPhysicalRecord(headerbuf, key, value, &key_offset,
  //                                      &blob_offset);
  // 更新blob file的文件大小和record数量
  // uint64_t size_put = headerbuf.size() + key.size() + value.size();
  bfile->BlobRecordAdded(size_put);
  total_blob_size_ += size_put;
  // 根据offset,value size,blob file number创建blob index
  CompressionType compression = kNoCompression;
  BlobIndex::EncodeBlob(index_entry, bfile->BlobFileNumber(), blob_offset,
                        value.size(), compression);
  return status;
}

Status DBImpl::GetBlobFileReader(
    const std::shared_ptr<blob_db::BlobFile>& bfile,
    std::shared_ptr<RandomAccessFileReader>* reader) {
  bool fresh_open = false;
  Status s = bfile->GetReader(env_, file_options_, reader, &fresh_open);
  if (s.ok() && fresh_open) {
...
Status DBImpl::GetRawBlobFromFile(const Slice& key, uint64_t file_number,
  ...
  if (offset < (BlobLogHeader::kSize + BlobLogRecord::kHeaderSize + key.size())) {
    return Status::NotFound("Invalid blob offset");
  }
  std::shared_ptr<blob_db::BlobFile> blob_file;
  {
  ...
  *compression_type = blob_file->GetCompressionType();
  ...
  //blob_file->CloseRandomAccessLocked();
  return Status::OK();
}

Status DBImpl::GetAllBlobFiles(std::set<uint64_t>* file_numbers) {
  assert(file_numbers != nullptr);
  std::vector<std::string> all_files;
  // 从db所在的dir中得到所有已存在的文件
  Status s = env_->GetChildren(initial_db_options_.blob_path, &all_files);
   if (!s.ok()) {
    return s;
  }
  // 筛选其中的blob file，解析出它们的file number
...
Status DBImpl::OpenAllBlobFiles() {
  std::set<uint64_t> file_numbers;
  // 得到所有已存在的blob file的file number
  Status s = GetAllBlobFiles(&file_numbers);
  if (!s.ok()) {
    return s;
  }
  // 把这些file number的最大值+1作为后续创建blob file编号的起点
  if (!file_numbers.empty()) {
    next_blob_number_.store(*file_numbers.rbegin() + 1);
  }
  // 根据file number创建blob_file指针，插入到map。
  for(auto& file_number : file_numbers) {
    std::shared_ptr<blob_db::BlobFile> blob_file =
        std::make_shared<blob_db::BlobFile>(
            nullptr, initial_db_options_.blob_path, file_number,
            initial_db_options_.info_log.get());
...
Status DBImpl::RecoverBlobFiles() {
  Status s;
  SequenceNumber sequencenumber=0;
  for (auto it : blob_files_) {
    auto blob_file = it.second;
    uint64_t block_offset = 0;
    block_offset += BlobLogHeader::kSize;
    std::shared_ptr<RandomAccessFileReader> reader;
    s = GetBlobFileReader(blob_file, &reader);
    if (!s.ok()) {
      return s;
    }
    while(/*(block_offset + BlobLogRecord::kHeaderSize < blob_file->file_size_ - BlobLogFooter::kSize)*/true) {
      // Allocate the buffer. This is safe in C++11
      std::string header_buf;
      AlignedBuf aligned_header_buf;

      Slice blob_header;
      BlobLogRecord header;
      {
        //StopWatch read_sw(env_, statistics_, BLOB_DB_BLOB_FILE_READ_MICROS);
        if (reader->use_direct_io()) {
          s = reader->Read(IOOptions(), block_offset,
                           BlobLogRecord::kHeaderSize, &blob_header, nullptr,
                           &aligned_header_buf);
        } else {
          header_buf.reserve(BlobLogRecord::kHeaderSize);
          s = reader->Read(IOOptions(), block_offset,
                           BlobLogRecord::kHeaderSize, &blob_header,
      ...
      if (blob_header.size() != BlobLogRecord::kHeaderSize) {
        break;
      }
      s = header.DecodeHeaderFrom(blob_header);
      if (!s.ok()) {
        break;
      }
      const uint64_t key_size = header.GetKeySize();
      const uint64_t value_size = header.GetValueSize();

      const uint64_t record_size = sizeof(uint32_t) + key_size + value_size;
      const uint64_t record_offset =
          block_offset + BlobLogRecord::kHeaderSize - sizeof(uint32_t);
      ...
      Slice blob_record;
    {
      ...
      // RecordTick(statistics_, BLOB_DB_BLOB_FILE_BYTES_READ,
      // blob_record.size());
    }
      if (!s.ok()) {
        break;
      ...
      if (blob_record.size() != record_size) {
        // return Status::Corruption("Failed to retrieve blob from blob
        // index.");
        break;
      }
      Slice crc_slice(blob_record.data(), sizeof(uint32_t));
      Slice key(blob_record.data() + sizeof(uint32_t), key_size);
      ...
      BlobIndex::EncodeBlob(
          &index_entry, blob_file->BlobFileNumber(),
          block_offset + BlobLogRecord::kHeaderSize + key_size, value_size,
          compression);
      block_offset += BlobLogRecord::kHeaderSize + key_size + value_size;
      WriteBatch batch;
      batch.Clear();
      s = WriteBatchInternal::PutBlobIndex(&batch, 0, key, index_entry);
      if (!s.ok()) {
        return s;
      }
      ...
      s = WriteBatchInternal::InsertInto(
          ...
          &sequencenumber /*sequence number*/, nullptr /*has valid_writes*/,
