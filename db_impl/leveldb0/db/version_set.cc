...
static Iterator* GetIterByGlobalIndex(void* arg, const ReadOptions& options,
                                      const Slice& file_value) {
  TableCache* cache = reinterpret_cast<TableCache*>(arg);
  Cache::Handle* handle = nullptr;
  Iterator* iter;
  GlobalIndex::SkipListItem* found_item =
      reinterpret_cast<GlobalIndex::SkipListItem*>(
          const_cast<char*>(file_value.data()));

  // Slice value =
  // *reinterpret_cast<Slice*>(const_cast<char*>(file_value.data()));
  // // uint64_t size = DecodeFixed64(file_value.data() + 8);
  // // Slice value = Slice(data, size);

  // uint64_t file_number = DecodeFixed64(file_value.data() + sizeof(Slice));
  // uint64_t file_size =
  //     DecodeFixed64(file_value.data() + sizeof(Slice) + sizeof(uint64_t));
  Status s =
      cache->FindTable(found_item->file_number, found_item->file_size, &handle);
  if (s.ok()) {
    Table* t = reinterpret_cast<TableAndFile*>(cache->cache_->Value(handle))->table;
    iter = t->GetIterByIndex(options, found_item->value);
    cache->cache_->Release(handle);
  }
  return iter;
}

Iterator* Version::GlobalIndexConcatenatingIterator(
    const ReadOptions& options, Iterator* global_index_iter) const {
  return NewTwoLevelIterator(
      global_index_iter, &GetIterByGlobalIndex,
      vset_->table_cache_, options);
}

void Version::AddGlobalIndexIterators(const ReadOptions& options,
                                      std::vector<Iterator*>* iters,
                                      leveldb::GlobalIndex* global_index_) {
  // Merge all level zero files together since they may overlap
  for (size_t i = 0; i < global_index_->index_files_level0.size(); i++) {
    iters->push_back(GlobalIndexConcatenatingIterator(
        options,
        new GlobalIndexIterator(global_index_->index_files_level0[i])));
  }
  ...
  for (int level = 0; level < config::kNumLevels - 1; level++) {
    if (!global_index_->index_files_[level]->IsEmpty()) {
      iters->push_back(GlobalIndexConcatenatingIterator(
          options,
          new GlobalIndexIterator(global_index_->index_files_[level])));
    }
  }
}
...
GlobalIndex::SkipListItem* GlobalIndex::InitItem(Slice key, Slice value,
  ...
  char* value_data = (char*)arena_.Allocate(value.size());
  memcpy(value_data, value.data(), value.size());
  item_ptr->value = Slice(value_data, value.size());
...
void GlobalIndex::SkipListGlobalIndexBuilder(
    const ReadOptions& options, Iterator* iiter, FilterBlockReader* filter,
    uint64_t file_number, uint64_t file_size, GITable** gitable_,
    GITable::Node** next_level_node, GITable::Node* bound_node,
    bool is_first, GITable::Iterator** next_level_iter) {
  // TODO
  ...
  if (iiter->Valid()) {
    item_ptr = InitItem(iiter->key(), iiter->value(), file_number, file_size);
  ...
  Iterator* block_iter;
  Status s;
  s = vset->table_cache_->GetIterByIndexBlock(options, item_ptr->file_number,
                                              item_ptr->file_size, &block_iter,
                                              item_ptr->value);
  if (s.ok()) 
  ...
  //        (*next_level_node)->Next(0)->GetHeight() > 0);
  // std::cout << "insert: key:" << item_ptr->key.ToString()
  //           << " ptr: " << (*next_level_node)->key.key.ToString() << std::endl;
  (*gitable_)->SeqInsert(*item_ptr, bound_node);
  // (*gitable_)->Insert(*item_ptr);
...
void GlobalIndex::AddFile(int level, const ReadOptions& options,
  ...
  // std::cout << "AddFile: " << level << " fnumber:" << file_number << std::endl;
  ...
  vset->table_cache_->IndexBlockGet(file_number, file_size, &iiter, &filter);
  iiter->SeekToFirst();
  ...
  if (level == 0) {
    ...
    SkipListGlobalIndexBuilder(options, iiter, filter, file_number, file_size,
                               &gitable, &next_level_node, bound_node, true,
                               &next_level_iter);
    ...
    // next_level_iter->SeekToFirst();
    std::string abc = "abc";
    SkipListItem* item_ptr = new SkipListItem(smallest.Encode());
    gitable_iter->Seek(*item_ptr);
    bound_node = gitable_iter->node_;
    if (bound_node == nullptr) bound_node = gitable->tail_;
    ...
    SkipListGlobalIndexBuilder(options, iiter, filter, file_number, file_size,
                               &gitable, &next_level_node, bound_node, is_first,
                               &next_level_iter);
    // TODO: 修改同一层指针
    SkipListItem* large_item_ptr = new SkipListItem(largest.Encode());
    gitable_iter = new GITable::Iterator(gitable);
    ...
    while (vset_->icmp_.Compare((last_level_iter->key()).key, largest.Encode()) <= 0) {
...
void GlobalIndex::DeleteFile(int level, uint64_t file_number,
                             ...
                             InternalKey smallest, InternalKey largest) {
  // std::cout << "DeleteFile: " << level << " fnumber" << file_number << std::endl;
  // std::cout << smallest.Encode().ToString() << "~"
  //           << largest.Encode().ToString() << std::endl;
  ...
  GITable::Node* next_level_node = nullptr;
  SkipListItem smallest_item = SkipListItem(smallest.Encode());
  SkipListItem largest_item = SkipListItem(largest.Encode());
  ...
  if (level != 0) {
    ...
    //   delete node;
    // }
    assert(list != nullptr); // 这里断言不会把某个level全部删掉，但严格来说是可能出现的情况
    // TODO: 处理层间指针
    // DONE.
    if (level == 1) {
    ...
    if (is_head && is_last) {
      ...
      while (index_iter->Valid() &&
            vset_->icmp_.Compare(index_iter->key().key, next_node->key.key) <=
                0) {
        index_iter->key().SetNextNode(prev_node);
        index_iter->Next();
      }
    ...
    for (std::vector<GITable*>::iterator iiter = index_files_level0.begin();
      ...
      if (index_iter->Valid() && index_iter->key().file_number == file_number) {
        ...
        if (i == 0) {
          ...
          if (i == index_files_level0.size() - 1) {
            // last SST in level 0.
            next_gitable = index_files_[0];
          } else {
            // other.
            next_gitable = index_files_level0[i + 1];
          }
          next_gitable_iter = new GITable::Iterator(next_gitable);
          index_iter->SeekToFirst();
...
void GlobalIndex::GlobalIndexBuilder(
  ...
  for (int level = config::kNumLevels - 1; level > 0; level--) {
    ...
    for (uint32_t i = 0; i < files_[level].size(); i++) {
      ...
      FilterBlockReader* filter;
      // get the index block of this sstable into iiter
      vset->table_cache_->IndexBlockGet(f->number, f->file_size, &iiter, &filter);
      // insert this index block into the skiplist global index table
      SkipListGlobalIndexBuilder(options, iiter, filter, f->number,
                                 f->file_size, &gitable_, &next_level_node,
                                 gitable_->tail_, is_first, &skiplist_iter_ptr);
  ...
  if (!tmp.empty()) {
    ...
    for (uint32_t i = 0; i < tmp.size(); i++) {
      ...
      SkipListGlobalIndexBuilder(
          options, iiter, filter, f->number, f->file_size, &gitable_,
          &next_level_node, gitable_->tail_, true, &skiplist_iter_ptr);
      S.push(gitable_);
      skiplist_iter_ptr = new GITable::Iterator(gitable_);
      (*skiplist_iter_ptr).SeekToFirst();
      delete iiter;
...
void GlobalIndex::SearchGITable(const ReadOptions& options, Slice internal_key,
                                SkipListItem* item_ptr, GITable* gitable_,
                                void** next_level_, void* arg,
  ...
  if (*next_level_ != nullptr) {
    ...
    // if ((*next_level_)->Next(0)->GetHeight() == 0) {
    //   // *next_level_ = (GITable::Node*)(*next_level_)->key.next_level_node;
    //   *next_level_ = nullptr;
    //   return;  // not found
    // }
  ...
  // index_iter->Seek(*item_ptr, *next_level_);
  if(index_iter->Valid()) {
    // Found.
    
    found_item = &index_iter->key();
    // std::cout << "my filenumber" << found_item->file_number << std::endl;
    VersionSet* vset = vset_;
    ...
    */
    s = vset->table_cache_->GetByIndexBlock(
        options, found_item->file_number, found_item->file_size, internal_key,
        found_item->value, arg, handle_result);
    // found;
  } else {
    // *next_level_ = nullptr;
...
bool GlobalIndex::GetFromGlobalIndex(const ReadOptions& options, Slice user_key,
                                 Slice internal_key, Status* my_s, void* arg,
                                 void (*handle_result)(void*, const Slice&,
                                                       const Slice&)) {
  // TODO:
  ...
  SkipListItem* item_ptr = new SkipListItem(internal_key);
  ...
  for (uint32_t i = 0; i < index_files_level0.size(); i++) {
    ...
    SearchGITable(options, internal_key, item_ptr, index_files_level0[i],
  ...
  for (uint32_t i = 0; i < index_files_.size(); i++) {
    ...
    SearchGITable(options, internal_key, item_ptr, index_files_[i],
                  &next_level_, saver, handle_result);
    switch (saver->state) {
      case kFound:
        *my_s = Status::OK();
...
void Version::ForEachOverlapping(Slice user_key, Slice internal_key, void* arg,
  ...
  if (!tmp.empty()) {
    ...
    for (uint32_t i = 0; i < tmp.size(); i++) {
      ...
      if (!(*func)(arg, 0, tmp[i])) {
        // std::cout << "file number: " << tmp[i]->number << std::endl;
        return;
      }
    }
  }
...
Status Version::Get(const ReadOptions& options, const LookupKey& k,
  ...
  if (!global_index_->global_index_exists_) {
    ...
    // check level0
    global_index_->CheckLevel0(options, files_);
  }
  ...
  my_saver.value = value;
  Status my_s;
  bool found;

  // start_time = clock();
  if (global_index_->global_index_exists_)
    // GetFromGlobalIndexL0(k.user_key(), k.internal_key(), &state, &State::Match,
    //                      global_index_, options, &my_s, &my_saver, SaveValue);
    found = global_index_->GetFromGlobalIndex(
        options, k.user_key(), k.internal_key(), &my_s, &my_saver, SaveValue);
  else
    ForEachOverlapping(state.saver.user_key, state.ikey, &state, &State::Match);

  // found = global_index_->GetFromGlobalIndex(
  //     options, k.user_key(), k.internal_key(), &my_s, &my_saver, SaveValue);
  // // std::cout << "my_value: " << *value << std::endl;
  ...
  // if (op_count >= 1000000) {
  //   std::cout << "My searching time of 1000000 ops is: "
  ...
  // if (my_value != *value || found != state.found) {
  ...
  if (global_index_->global_index_exists_) 
    return Status::OK();
  // return found ? my_s : Status::NotFound(Slice());
  else
...
Compaction* VersionSet::PickCompaction() {
  ...
  const bool size_compaction = options_->enable_compaction && (current_->compaction_score_ >= 1);
  const bool seek_compaction = false;
  // const bool seek_compaction =
  //     options_->enable_compaction && (current_->file_to_compact_ != nullptr);
  if (size_compaction) {
