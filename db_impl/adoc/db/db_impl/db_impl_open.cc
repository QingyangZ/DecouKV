...
Status DBImpl::NewDB(std::vector<std::string>* new_filenames) {
  ...
  const std::string manifest = DescriptorFileName(dbname_, 1);
  {
    ...
    std::ofstream fout1("/mnt/sde/zqy/data/adoc/compaction.txt");
    fout1 << env_->NowMicros() << std::endl;
    fout1.close();
    std::ofstream fout2("/mnt/sde/zqy/data/adoc/flush.txt");
    fout2 << env_->NowMicros() << std::endl;
    fout2.close();
    std::ofstream fout3("/mnt/sde/zqy/data/adoc/l0l1_compaction.txt");
