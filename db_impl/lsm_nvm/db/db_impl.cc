...
: env_(raw_options.env),
  ...
  options_(SanitizeOptions(dbname_disk, &internal_comparator_,
    ...
    std::ofstream fout1("/mnt/sdc/zqy/rocksdb/novelsm/Flush_Data.txt");
    fout1.close();
    std::ofstream fout2("/mnt/sdc/zqy/rocksdb/novelsm/Compaction_Data.txt");
    fout2.close();
...
void DBImpl::BackgroundCompaction() {
    ...
    if (imm_ != NULL) {
        ...
        auto duration1 = std::chrono::duration_cast<std::chrono::milliseconds>(now_time - start_time_);
        std::ofstream fout("/mnt/sdc/zqy/rocksdb/novelsm/Flush_Data.txt", std::ios::app);
        // fout << duration.count() << ',' << 1 << std::endl;
        
        CompactBottomMemTable();

        now_time = std::chrono::system_clock::now();
        auto duration2 = std::chrono::duration_cast<std::chrono::milliseconds>(now_time - start_time_);
        fout << duration1.count() << ',' << duration2.count() << std::endl;
        fout.close();
        return;
    }
    ...
    if (c == NULL) {
        ...
        auto now_time = std::chrono::system_clock::now();
        auto duration1 = std::chrono::duration_cast<std::chrono::milliseconds>(now_time - start_time_);
        std::ofstream fout("/mnt/sdc/zqy/rocksdb/novelsm/Compaction_Data.txt", std::ios::app);
        // fout << duration.count() << ',' << 1 << std::endl;
    ...
    *dbptr = NULL;
    ...
    if (s.ok() && impl->mem_ == NULL) {
        ...
        if (s.ok() && (impl->mem_ == NULL)) {
            ...
            WritableFile* lfile;
            //Save log in nvm for now
            s = options.env->NewWritableFile(LogFileName(dbname_mem, new_log_number),
                    &lfile);
            if (s.ok()) {
