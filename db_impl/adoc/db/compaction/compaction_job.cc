...
Status CompactionJob::Run() {
  ...
  if (compact_->compaction->output_level() == 0 || compact_->compaction->output_level() == 1) {
    std::ofstream fout("/mnt/sde/zqy/data/adoc/l0l1_compaction.txt",
                     std::ios::app);
    fout << start_micros << ',' << env_->NowMicros()
        << std::endl;
    fout.close();
  } else {
    std::ofstream fout("/mnt/sde/zqy/data/adoc/compaction.txt",
                     std::ios::app);
    fout << start_micros << ',' << env_->NowMicros()
        << std::endl;
    fout.close();
