...
void Compaction::AddInputDeletions(VersionEdit* out_edit) {
  ...
  for (size_t which = 0; which < num_input_levels(); which++) {
    for (size_t i = 0; i < inputs_[which].size(); i++) {
      ...
      if (column_family_data()->global_index->global_index_exists_) {
        column_family_data()->global_index->delete_files_.emplace(
            inputs_[which][i]->fd.GetNumber(),
            GlobalIndex::EditFile(level(which), inputs_[which][i]->smallest,
                                  inputs_[which][i]->largest));
      }
    }
  }
}
