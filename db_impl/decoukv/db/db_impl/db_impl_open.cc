...
Status DBImpl::WriteLevel0TableForRecovery(int job_id, ColumnFamilyData* cfd,
  ...
  meta.fd = FileDescriptor(versions_->NewFileNumber(), 1, 0);
  ReadOptions ro;
...
Status DBImpl::Open(const DBOptions& db_options, const std::string& dbname,
  ...
  *dbptr = nullptr;
  ...
  // tht update: 打开所有blob file
  impl->OpenAllBlobFiles();
  // if (!s.ok()) {
  //   delete impl;
  //   return s;
  // }
    ...
    if (s.ok()) {
      ...
      for (auto cf : column_families) {
        ...
        if (cfd != nullptr) {
          ...
          // modified by zqy
          cfd->global_index = new GlobalIndex;
          if (cfd->initial_cf_options().use_queue_monitor)
            cfd->key_table = new KeyTable(cfd->internal_comparator());
          else
            cfd->key_table = nullptr;
          impl->start_time_ = std::chrono::high_resolution_clock::now();
          // std::ofstream fout1("/mnt/sde/zqy/data/index/tuning.txt");
          // fout1.close();
          // std::ofstream fout2("/mnt/sde/zqy/data/index/queue_length.txt");
          // fout2.close();
          // std::ofstream fout1("MemData.txt");
          // fout1 << "time,L0 filenumber,arrive rate,service rate,write buffer
          // "
          //          "size, flush threads,compaction threads"
          //       << std::endl;
          // fout1.close();

          // std::ofstream fout2("L0Data.txt");
          // fout2 << "L0 compaction latency,L0 compaction number,L1 compaction "
          //          "number,arrive "
          //          "rate,service rate,L0 slowdown trigger,write buffer "
          //          "size,flush threads,compaction threads"
          //       << std::endl;
          // fout2.close();

          if (cfd->initial_cf_options().use_global_index) {
            clock_t start_time, end_time;
            ReadOptions read_options;
            std::cout << "index build:" << std::endl;
            start_time = clock();
            
            cfd->global_index->cfd_ = cfd;
            cfd->global_index->GlobalIndexBuilder(read_options,
                                                  cfd->current()->storage_info());
            cfd->global_index->max_file_size_for_l0_meta_pin_ =
                MaxFileSizeForL0MetaPin(cfd->current()->GetMutableCFOptions());
            end_time = clock();
            std::cout << "The run time is: "
                      << (double)(end_time - start_time) * 1000 / CLOCKS_PER_SEC << "ms"
                      << std::endl;

            std::cout << "use global index" << std::endl;
            cfd->global_index->global_index_exists_ = true;
            cfd->global_index_exist_ = true;
          }
        } else {
