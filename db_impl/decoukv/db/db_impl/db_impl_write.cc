...
Status DBImpl::WriteImpl(const WriteOptions& write_options,
  ...
  std::string index_entry_value;
  if (status.ok()) {
    // tht update:
    bool use_blob = true;
    if(use_blob) {
      // 依次处理write group中的每个writer
      for(auto* writer: write_group) {
        // 用于存放新的record
        std::string str = writer->batch->Data();
        Slice input(
            str.data() + WriteBatchInternal::kHeader,
            static_cast<size_t>(str.size() - WriteBatchInternal::kHeader));
        writer->batch->Clear();
        while(status.ok() && !input.empty()) {
          // 对于每个batch，依次读取其中的record
          status = ReadRecordFromWriteBatch(&input, &tag, &column_family, &key,
                                            &value, &blob, &xid);
          switch (tag) {
            case kTypeMerge:
            case kTypeValue:
              PutBlobValue(key,value,&index_entry_value,column_family);
              // 将key和blob index放入新batch
              status = WriteBatchInternal::PutBlobIndex(
                  writer->batch, column_family, key, index_entry_value);
              // status = WriteBatchInternal::PutBlobIndex(
              //     &newbatch, column_family, key, index_entry_value);
              break;
            case kTypeDeletion:
              // 如果是删除类型的数据，则不需要写入blob file，直接放入新batch
              status =
                  WriteBatchInternal::Delete(writer->batch, column_family, key);
              break;
            case kTypeSingleDeletion:
              status = WriteBatchInternal::SingleDelete(writer->batch,
                                                        column_family, key);
            default:
              break;
          }
        }
        // 更新writer的batch，后续写入到memtable的操作与原来相同
        //writer->batch = &newbatch;
      }
    }
...
Status DBImpl::PipelinedWriteImpl(const WriteOptions& write_options,
  ...
  if(use_blob) {
    // 用于存放新的record
    std::string str = w.batch->Data();
    Slice input(str.data() + WriteBatchInternal::kHeader,
                static_cast<size_t>(str.size() - WriteBatchInternal::kHeader));
    w.batch->Clear();
    while(status.ok() && !input.empty()) {
      // 对于每个batch，依次读取其中的record
      status = ReadRecordFromWriteBatch(&input, &tag, &column_family, &key, &value,
                                 &blob, &xid);
      switch (tag) {
        case kTypeMerge:
        case kTypeValue:
          PutBlobValue(key,value,&index_entry_value,column_family);
          // 将key和blob index放入新batch
          status = WriteBatchInternal::PutBlobIndex(w.batch, column_family, key,
                                                    index_entry_value);
          // status = WriteBatchInternal::PutBlobIndex(&newbatch, column_family,
          //                                           key, index_entry_value);
          break;
        case kTypeDeletion:
          // 如果是删除类型的数据，则不需要写入blob file，直接放入新batch
          status = WriteBatchInternal::Delete(w.batch,column_family,key);
          break;
        case kTypeSingleDeletion:
          status = WriteBatchInternal::SingleDelete(w.batch,column_family,key);
        default:
          break;
      }   
    // 更新writer的batch，后续写入到memtable的操作与原来相同
...
Status DBImpl::PreprocessWrite(const WriteOptions& write_options,
  ...
  if (UNLIKELY(status.ok() && (write_controller_.IsStopped() ||
      ...
      total_blob_size_ > write_options.nvm_stall_trigger*/) {
    PERF_TIMER_STOP(write_pre_and_post_process_time);
...
Status DBImpl::SwitchMemtable(ColumnFamilyData* cfd, WriteContext* context) {
  ...
  // s = CloseBlobFile(open_blob_file_);
  // if (!s.ok()) {
  //   return s;
  // }
  std::shared_ptr<BlobLogWriter> bwriter;
  s = CreateBlobFileAndWriter(false, ExpirationRange(), "create new blob file",
                              &open_blob_file_, &bwriter);
  if (!s.ok()) {
    return s;
  }
  RegisterBlobFile(open_blob_file_);
  new_mem->AddBlobFileNumber(open_blob_file_->BlobFileNumber());
