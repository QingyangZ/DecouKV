...
class MemTable {
 public:
  ...
  // modified by zqy
  bool GetByNode(const LookupKey& key, std::string* value,
                   std::string* timestamp, Status* s,
                   MergeContext* merge_context,
                   SequenceNumber* max_covering_tombstone_seq,
                   SequenceNumber* seq, const ReadOptions& read_opts,
                   void** next_node, ReadCallback* callback = nullptr,
                   bool* is_blob_index = nullptr, bool do_merge = true);

  bool GetByNode(const LookupKey& key, std::string* value,
                 std::string* timestamp, Status* s, MergeContext* merge_context,
                 SequenceNumber* max_covering_tombstone_seq,
                 const ReadOptions& read_opts, void** next_node,
                 ReadCallback* callback = nullptr,
                 bool* is_blob_index = nullptr, bool do_merge = true) {
    SequenceNumber seq;
    return GetByNode(key, value, timestamp, s, merge_context,
                     max_covering_tombstone_seq, &seq, read_opts, next_node,
                     callback, is_blob_index, do_merge);
  }

  int count_table_ = 1;
  std::mutex mutex_;
  bool is_schflushed_ = false;
  bool is_flushed_ = false;
  void Compact(SequenceNumber seq, MemTable* last_table);
  // store the blob file numbers
  std::vector<uint64_t> blob_files_;
  std::vector<MemTable*> memtables_;

  void AddBlobFileNumber(uint64_t blob_file_number) {
    blob_files_.emplace_back(blob_file_number);
  }
 ...
 private:
  ...
  // modified by zqy
  void GetFromTableByNode(const LookupKey& key,
                          SequenceNumber max_covering_tombstone_seq,
